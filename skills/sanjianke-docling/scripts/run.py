#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""文档问答 —— 零安装版（走平台 file_qa/chat）。

原先要用这个 Skill，得先 pip install docling 和它的 PyTorch、再从 Hugging Face
下版面分析 / 表格结构 / OCR 的一堆模型权重，还要留出可观的磁盘与内存。
只想「问一份文档几个问题」的话，这套前置太重了。

现在本机只要求 Python 3.8+：把文档的公网地址和问题提交给 api.a7w.cn，
直接拿回答案。
...
【file_qa 只吃公网 HTTP(S) 地址，不支持本地文件上传】
    ...
    形如 https://.../报告.pdf ；你本机的 C:\\Users\\...\\报告.pdf 是传不进去的。

【怎么先拿到公网 URL】（下面「零安装用法」里也有一份，任选一种）
    1) 对象存储：阿里云 OSS / 腾讯云 COS / 七牛 / MinIO / AWS S3，
       上传后取**带签名的临时直链**或公开读的直链。最稳，适合批量。
    2) 临时分享站（几十 MB 的小文件够用）：
       curl -F "file=@报告.pdf" https://0x0.st          # 返回一行纯文本直链
       curl -T 报告.pdf https://transfer.sh/报告.pdf     # 返回一行直链
       curl -F "file=@报告.pdf" https://tmpfiles.org/api/v1/upload
...
用法
    python3 run.py --url https://example.com/报告.pdf "这份文档讲了什么"
    python3 run.py --url https://example.com/a.pdf --url https://example.com/b.docx "对比两份文档的关键差异"
    python3 run.py --url https://example.com/规格书.pdf "把参数表整理成列表" -o 答案.md
    python3 run.py --url https://example.com/长文档.pdf "全文摘要" --mode task   # 转异步，避免长连接被掐
...
def main():
    ap = argparse.ArgumentParser(
        description="文档问答：把公网文档地址和问题交给 api.a7w.cn（零安装）",
