# 三剪客 · 可视化 AI 工作流编排 Skill

在画布上搭出可调试、可发布的 AI 工作流，模型侧一把 Key 接 75 个在架模型与 21 个生成应用

---

## 这个包解决什么

把「输入 → 检索 → 模型 → 工具 → 输出」这条链路从代码里搬到画布上：拖节点、连线、
实时调试。模型提供方整排收成**一个 `base_url`、一把 Key、一份账单** —— 
`https://api.a7w.cn/api/v1` 兼容 OpenAI 协议，换 `model` 字符串就是换模型；
出图、出视频、数字人、配音、音乐、文档问答走统一的 `/api/v1/apps/{app}/{api}`。

**不需要逐家注册、逐家充值、逐家对账。** 实测 75 个在架模型（23 家厂商）与 21 个生成应用，
换算口径统一：1 元 = 100 点，先冻结后结算，失败全额退回。
...
```bash
# 1) 到 https://api.a7w.cn/ 注册领 Key，然后
export A7W_API_KEY=sk-你的key      # Windows: $env:A7W_API_KEY="sk-你的key"

# 2) 画布里模型节点的 API Base 填 https://api.a7w.cn/api/v1，模型名从清单里选
...
零依赖客户端（只用 Python 标准库）：

```bash
python3 scripts/a7w.py whoami                # 验 Key
python3 scripts/a7w.py apps                  # 21 个生成应用
python3 scripts/a7w.py schema voice_tts      # 接口、参数、同步/异步
