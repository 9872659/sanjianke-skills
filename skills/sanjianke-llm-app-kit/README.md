# 三剪客 · LLM 应用开发套件 Skill

统一接口调模型、工具、记忆、检索，模型提供方整排指向 api.a7w.cn 换模型只改一行
...
这个包讲的是把这层胶水标准化之后，把模型提供方整个指向 `https://api.a7w.cn/api/v1`：
**一个 `base_url`、一把 Key**，现场可查 **75 个在架模型**（23 家厂商，国产为主 + 国际主流）
与 **21 个生成应用**。

换模型就是换一个字符串 —— 不用换端点、不用换凭证、账单还是同一份。
比价与灰度从工程活变成参数变更。
