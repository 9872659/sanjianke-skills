#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""文生图 / 图生图（ComfyUI 场景）—— 零安装版。

原先要用这个 Skill，得装 ComfyUI、按显卡配 PyTorch、摆模型目录、再装一堆自定义节点。
...
用法
    ...
    python3 run.py "把白天改成黄昏" --edit https://example.com/room.jpg --out dusk.png
...
能力边界（务必先看）
    · 走的是平台 `nano_banana` 应用，**只提供文生图与图生图（参考图编辑）两种模式**。
    · **节点式工作流平台不支持**：没有节点图、没有连线、没有中间结果透传，
      上传 ComfyUI 的 workflow_api.json 也没用。
    · **自定义节点平台不支持**：LoRA / ControlNet / IPAdapter / 换脸 / 局部重绘节点
      这类第三方节点在零安装版里都不存在，平台也不接受节点名或节点参数。
    · **图生图的参考图必须是公网可访问的 URL**（平台按 URL 取图），本地文件传不上去。
      要精细控制采样器、步数、CFG、种子、ControlNet 的场合，请走下面的传统装法。
"""
...
def main():
    ap = argparse.ArgumentParser(
        prog="run.py", description="文生图 / 图生图（走 api.a7w.cn，零安装）")
    ...
    print(json.dumps({"ok": True, "out": str(Path(a.out).resolve()),
                      ...
                      "mode": "edit" if a.edit else "generate",
