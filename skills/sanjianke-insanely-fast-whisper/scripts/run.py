#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""极速语音转写（insanely-fast-whisper）—— 零安装版。

原先要用这个 Skill，得先有 NVIDIA 显卡或 Apple Silicon：装 PyTorch、装 CUDA 版
torch、编译 flash-attn，要做说话人分离还得单独配 Hugging Face Token；机器不对
就一步都走不下去。现在这些都不用了：音频直接上传到 api.a7w.cn 转写，
本机只要求 Python 3.8+，**不用显卡、不用 flash-attn、不用下模型**。
...
能力边界（这条零安装路线做不到的，如实写在这里）
    · 不做说话人分离。平台 stt 接口不返回 speaker 标记，结果里没有「谁说的」；
      上游那个靠 HF Token + pyannote 的分离能力，这里没有替代品。
    · 没有 word 级时间戳。带时间戳时返回的是**逐字**时间戳（中文一字一条，
      60 字音频返回 60 条），比上游的 chunk 更细、比 word 更碎；本脚本会把它
      合并成正常人能读的字幕行。
    · 不做 `--task translate`（非英语翻成英文）。平台 stt 接口没有该参数，
      只做原语种转写。
    · 不暴露 beam size / 温度 / VAD 阈值这类解码参数。
    · 面向离线整段文件，不做流式实时转写。
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="极速语音转写 / 出字幕（走 api.a7w.cn，零安装、不需要显卡）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="例：python3 run.py 访谈.mp3 --srt\n"
               "    python3 run.py --url https://example.com/a.mp3 --lang zh")
