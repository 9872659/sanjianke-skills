...
你手上一堆内部文档——产品说明、客服话术、规章流程——同事反复来问同样的问题。

要做的事很清楚：**把这些文档灌成一个知识库，再套一层问答界面，让人直接问、它带出处地回答。**
麻烦的从来不是「写几行代码调一次模型」，而是**两件事同时要接**：对话模型，和检索这条链。

`api.a7w.cn` 把这两件事收成**一个 base_url、一个 Key、一份账单**。

| 你最关心 | 答案 |
|---|---|
| 多少钱 | **1 元 = 100 点**；文本按点数/百万 tokens，文档问答按点数/次 |
| 要多久 | 文本同步返回；长文档问答是异步任务，**查询免费** |
| 要装什么 | **什么都不用装**。包里自带零依赖客户端，或者直接用 `curl` |
| 能接什么 | **75 个在架模型** + **21 个生成应用**（含文档问答 `file_qa`） |
...
**知识库这一层只需要一个「OpenAI 兼容端点」的配置项。** 填这两行：
...
返回里有 `choices[0].message.content` 就说明问答链路这一端是通的。

### 第三步：检索这一端——先看平台上有什么

**不要凭印象写模型名或接口名。** 平台的能力清单是实时接口给的：

```bash
# 21 个生成应用（含文档问答 file_qa）
python3 scripts/a7w.py apps

# 每个应用的接口、参数、同步/异步与真实价
...
`schema` 的输出会标注 `call_type`（1 = 同步，2 = 异步）与 `method`，
**参数表以它为准**，别照抄教程里的字段名。

### 第四步：长文档问答直接走 `file_qa` 应用

不想自己搭切片与索引时，`file_qa` 就是现成的文档问答入口：
...
### 第五步：自建知识库时，把两条链都指向 a7w

自建知识库时你会有两个模型角色，**都指向同一个 base_url、用同一把 Key**：

| 角色 | 用途 | 从哪拿名字 |
|---|---|
| 对话模型 | 生成回答 | `GET /api/v1/models` |
| 检索问答 | 文档问答与要点抽取 | `python3 scripts/a7w.py schema file_qa` |

这样做的直接好处：**账单只有一份，对账只有一处，换模型只改一个字符串。**
...
| 环节 | 要做什么 | 常见做法 |
|---|---|
| **接入** | 把文档变成可检索的内容 | 公网 URL 交给 `file_qa`，或自建切片流程 |
| **检索** | 提问时先捞出相关片段 | 由知识库平台或 `file_qa` 承担 |
| **生成** | 拿片段 + 问题让模型回答 | `POST /api/v1/chat/completions` |
| **对外** | 给内部同事或外部系统用 | 平台自带界面，或按 OpenAPI 接自己的系统 |
...
| **文档问答** | 应用 `file_qa` |
| 语音与字幕 | 应用 `voice_tts`（含语音转文字） |

> 清单会变（实测 75 个模型 / 23 家厂商、21 个应用）。
> **调用前现场跑一次接口**，不要把自己的逻辑绑在某个名字一定存在上。
...
| 素材入参 | 文档 / 图片 / 音频一律用**公网可访问的 URL**，不支持本地路径、不支持 Base64 |
| 凭证 | 只需**你自己**的 API Key；包里不内嵌任何密钥 |
| 模型名 | 从 `/api/v1/models` 拿，不要凭印象写 |
